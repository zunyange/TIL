...
naver.maps.Util.ClassExtend(MarkerClustering, naver.maps.OverlayView, {
   ...
   * @param {10} size 클러스터를 구성할 그리드 크기
   */
  ...
  _createClusters: function () {
    ...
    for (let i = 0, ii = markers.length; i < ii; i++) {
      ...
      let closestCluster = this._getClosestCluster(position);
  ...
  _getClosestCluster: function (position) {
    ...
    for (let i = 0, ii = clusters.length; i < ii; i++) {
      ...
      if (cluster.isInBounds(position)) {
        let delta = proj.getDistance(center, position);
...
Cluster.prototype = {
  ...
  destroy: function () {
    ...
    let members = this._clusterMember;
  ...
  enableClickZoom: function () {
    ...
    let map = this._markerClusterer.getMap();
  ...
  checkByZoomAndMinClusterSize: function () {
    let clusterer = this._markerClusterer,
  ...
  updateCount: function () {
    let stylingFunction = this._markerClusterer.getStylingFunction();
  ...
  _calcBounds: function (position) {
    ...
    let max_px_x = Math.min(map_max_px.x, max_px.x),
  ...
  _getIndex: function (count) {
    let indexGenerator = this._markerClusterer.getIndexGenerator();
